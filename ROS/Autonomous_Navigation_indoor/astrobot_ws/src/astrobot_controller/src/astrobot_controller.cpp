...
double robot_vel_x, robot_vel_y, robot_vel_z = 0;
/*------------ robot odometry ------------*/
...
/*----------- recieve motor speed -------*/
void speed_vector(const geometry_msgs::Vector3Stamped &vector_speed)
{
    left_motor_speed = vector_speed.vector.x;
    right_motor_speed = vector_speed.vector.y;
    gZ = vector_speed.vector.z;
}
int main(int argc, char **argv)
{
  
    /*---------- controller node --------*/
    ros::init(argc, argv, "astrobot_controller");
    ros::NodeHandle astrobot;
    ros::Publisher odom_pub = astrobot.advertise<nav_msgs::Odometry>("odom", 100);
    ros::Subscriber vector_speed = astrobot.subscribe("vector_speed", 50, speed_vector);
    ...
    while (astrobot.ok())
    {
        ...
        dth = ((right_motor_speed - left_motor_speed) * dt) / wheel_base;

        vx = (dt == 0) ? 0 : (left_motor_speed + right_motor_speed) / 2;
        vth = (dt == 0) ? 0 : (right_motor_speed - left_motor_speed) / wheel_base;
